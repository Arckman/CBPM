 ...
 * @version $Revision: 1.34 $ $Date: 2007/11/29 10:16:30 $
 */
  ...
  public void read(BpelProcessDefinition processDefinition, ProcessArchive archive) {
    ...
    try {
      ...
      if(!JbpmConfiguration.getVersionControlManager().checkProcessDeployed(processDefinition.getName())){
	      ...
	      an.doGraph();
	      an.doAna();
    	  JbpmConfiguration.getVersionControlManager().addProcessMonitor(processDefinition,an);
      }else
    	  JbpmConfiguration.getVersionControlManager().setDynamicUpdatable(true);
    }
    catch (SAXException e) {
      problemHandler.add(new Problem(Problem.LEVEL_ERROR, "bpel document contains invalid xml: "
          + location, e));
