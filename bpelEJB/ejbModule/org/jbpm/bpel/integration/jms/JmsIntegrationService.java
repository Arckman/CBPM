 ...
 * @version $Revision: 1.20 $ $Date: 2008/01/30 07:18:22 $
 */
...
//      //========================================add root and parent information===========================
      InstanceMonitor im=JbpmConfiguration.getVersionControlManager().getProcessMonitor(token.getProcessInstance().getProcessDefinition().getName())
    		  .getInstanceMonitor(token.getProcessInstance().getId());
      if(im.getParentMonitorName()==null){
    	  inputParts.put("rootMonitorName", im.getPm().getProcessName());
    	  inputParts.put("rootId", im.getInstanceId());
      }else{
    	  inputParts.put("rootMonitorName", im.getRootMonitorName());
    	  inputParts.put("rootId", im.getRootInstanceId());
      }
      inputParts.put("parentMonitorName", im.getPm().getProcessName());
      inputParts.put("parentId", im.getInstanceId());
