 ...
 * @version $Revision: 1.26 $ $Date: 2007/11/25 13:06:55 $
 */
public class SoapHandler implements Handler {
  ...
  public boolean handleRequest(MessageContext messageContext) throws JAXRPCException,
     ...
     */
//	  if(JbpmConfiguration.getVersionControlManager().getStrategy().equals(MonitorConstants.STATRGY_CONCURRENT))
//	  integrationControl=null;
    if (integrationControl == null) {
      ...
      //===============================try to change intergrationControl============
//      if(integrationControl!=null&&JbpmConfiguration.getVersionControlManager().getStrategy().equals(MonitorConstants.STATRGY_CONCURRENT)){
//    	  JbpmContext jbpmContext = JbpmConfiguration.getInstance().createJbpmContext();
//    	  String processName;
//    	  try{
//    		  BpelProcessDefinition processDefinition = integrationControl.getDeploymentDescriptor().findProcessDefinition(jbpmContext);
//    		  processName=processDefinition.getName();
//    	  }finally{
//    		  jbpmContext.close();
//    	  }
//    	  ProcessMonitor pm=JbpmConfiguration.getVersionControlManager().getProcessMonitor(processName);
//    	  SOAPMessage soapMessage = ((SOAPMessageContext) messageContext).getMessage();
//    	  String rootMonitorName=null;
//    	  long rootId=0;
//    	  SOAPPart soapPart=soapMessage.getSOAPPart();
//    	    SOAPEnvelope soapEnvelop;
//			try {
//				soapEnvelop = soapPart.getEnvelope();
//				SOAPBody soapBody=soapEnvelop.getBody();
//				Iterator i=soapBody.getChildElements(soapEnvelop.createName("rootMonitorName"));
//				while(i.hasNext()){
//					SOAPBodyElementDoc element=(SOAPBodyElementDoc) i.next();
//					rootMonitorName=element.getValue();
//				}
//				i=soapBody.getChildElements(soapEnvelop.createName("rootId"));
//				while(i.hasNext()){
//					SOAPBodyElementDoc element=(SOAPBodyElementDoc) i.next();
//					rootId=Long.valueOf(element.getValue());
//				}
//			} catch (SOAPException e) {
//				// TODO Auto-generated catch block
//				e.printStackTrace();
//			}
////			if(pm.checkHasPast(rootMonitorName, rootId)){
//			if(true){//for test
//	    	  IntegrationControl ic=pm.getOldIntegrationControl();
//	    	  integrationControl=ic;
//			}
//      }
    }
  ...
  protected ObjectMessage sendRequest(SOAPMessage soapMessage, Session jmsSession,
    ...
    //==============================server get soap message,and re-send jms message=============================
    SOAPPart soapPart=soapMessage.getSOAPPart();
    ...
    while(i.hasNext()){
    	SOAPBodyElementDoc element=(SOAPBodyElementDoc) i.next();
    	requestParts.put("rootMonitorName", element.getValue());
    }
    i=soapBody.getChildElements(soapEnvelop.createName("rootId"));
    while(i.hasNext()){
    	SOAPBodyElementDoc element=(SOAPBodyElementDoc) i.next();
    	requestParts.put("rootId", element.getValue());
    }
    i=soapBody.getChildElements(soapEnvelop.createName("parentMonitorName"));
    while(i.hasNext()){
    	SOAPBodyElementDoc element=(SOAPBodyElementDoc) i.next();
    	requestParts.put("parentMonitorName", element.getValue());
    }
    i=soapBody.getChildElements(soapEnvelop.createName("parentId"));
    while(i.hasNext()){
    	SOAPBodyElementDoc element=(SOAPBodyElementDoc) i.next();
    	requestParts.put("parentId", element.getValue());
    }
